...
Agent.prototype._reply = function(request, err, message) {
  ...
  if (err) {
    backend.emit('replyError', agent, err, request);
...
Agent.prototype._handleMessage = function(request, callback) {
  try {
    ...
    switch (request.a) {
      ...
      case 'nc':
        return this._fetchChangelog(request.c, request.d, request.o, callback);
      default:
...
Agent.prototype._fetchChangelog = function (collection, id, options, callback) {
  this.backend.fetchChangelog(this, collection, id, options, callback);
};
