...
function Doc(connection, collection, id) {
  ...
  // Whether to emit 'op' event for each incoming op even if they're received within one package.
  // Setting to true may be reasonable if handling op may result in submitting further ops. Defaults to true.
  this.handleOpsSeparately = true;
  ...
  // Whether op is currently emitted. Used to check if no synchronous ops are
  // submitted with handleOpsSeparately flag set to false
  this.emittingOp = false;
}
emitter.mixin(Doc);
  ...
  if (this.wantSubscribe) {
    if (callbacks.length) {
      this.subscribe(function(err) {
        var consumed = callEach(callbacks, err);
        if (err && !consumed) doc.emit('error', err);
      });
      return;
    }
    this.subscribe();
...
Doc.prototype.subscribe = function(callback, options) {
  this.wantSubscribe = true;

  if (options && typeof options.handleOpsSeparately === 'boolean') {
    this.handleOpsSeparately = options.handleOpsSeparately;
  }
...
Doc.prototype._otApply = function(op, options) {
  ...
  if (op.op) {
    ...
    if (this.handleOpsSeparately && !source && this.type === types.defaultType && op.op.length > 1) {
...
Doc.prototype._submit = function(op, options, callback) {
  ...
  if (op.op) {
    if (!this.type) {
      ...
      if (callback && callback(err) !== false) return;
      return this.emit('error', err);
    }
    ...
    if (this.handleOpsSeparately === false && this.emittingOp) {
      var err = new ShareDBError(4030, 'Cannot submit new op in op handler if not handling ops separately');
      if (callback && callback(err) !== false) return;
      return this.emit('error', err);
    }
  }
...
Doc.prototype._hardRollback = function(err) {
  ...
  this.fetch(function() {
    var consumed = callEach(callbacks, err);
    if (err && !consumed) return doc.emit('error', err);
  });
};
...
Doc.prototype._clearInflightOp = function(err) {
  ...
  this.inflightOp = null;
  var consumed = callbacks && callEach(callbacks, err);

  this.flush();
  this._emitNothingPending();

  if (err && !consumed) return this.emit('error', err);
};
...
function callEach(callbacks, err) {
  ...
  for (var i = 0; i < callbacks.length; i++) {
    ...
    if (callback) {
      consumed = callback(err) === false ? false : (consumed === null ? true : consumed);
    }
  }
  return consumed === null ? false : consumed;
}
...
Doc.prototype._handlePresence = function(err, presence) {
  ...
  if (!src) {
    ...
    if (this.inflightPresenceSeq === presence.seq) {
      ...
      this.inflightPresenceSeq = 0;
      var consumed = callbacks && callEach(callbacks, err);
      if (err && !consumed) this.emit('error', err);
      this.flush();
      this._emitNothingPending();
    }
    return;
