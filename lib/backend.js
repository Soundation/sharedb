...
Backend.prototype.MIDDLEWARE_ACTIONS = {
  ...
  submit: 'submit',
  // Changelog was received from the database and are about to be return to a client
...
Backend.prototype.fetchChangelog = function(agent, collection, id, options, callback) {
  var start = Date.now();
  var backend = this;

  backend.db.getChangelog(collection, id, options, function (error, data) {
    if(error) return callback(error);

    var changelog = new Changelog(id, data);
    ...
    backend.trigger(backend.MIDDLEWARE_ACTIONS.readChangelog, agent, request, function (error) {
      if (error) return callback(error);
      backend.emit('timing', 'fetchChangelog', Date.now() - start, request);
